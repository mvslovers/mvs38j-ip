...
#define	BITS_PER_OCTET	8	// number of bits in a byte
...
//  Structure passed to ipshowhdr, who fills it in 

struct	ip_cheat	{
	void	*buf;		// buffer addr
	int	hlen;		// IP header length in bytes
	int	paclen;		// total packet length in bytes
	void	*data;		// beginning of data
	int	datalen;	// length of data in bytes
};
typedef	struct ip_cheat	ipcheat;
...
//  Maximum return string length for ip2dot; IPv4
#define	IP_MAXDOT	sizeof("123.456.789.123")

/* IP process info */

#define	IPSTK		512		// stack size for IP process
#define	IPPRI		100		// IP runs at high priority
#define	IPNAM		"ip-in ipproc"// name of IP process
...
#include <ether.h>		// struct ep
#include <netif.h>		// struct netif

USHORT		cksum(USHORT *, unsigned);
USHORT		ipcksum(void *);	// @ IP frame
int		ipdstopts(struct netif *, struct ep *);
void		ipfinit(void);
void		ipftimer(int);
struct ip	*iph2net(struct ip *);
struct ip	*ipnet2h(struct ip *);
PROCESS		ipproc();
int		ipputp(unsigned, IPaddr, struct ep *);
struct ep	*ipreass(struct ep *);
int		ipsend(IPaddr, struct ep *, unsigned, u_char, u_char, u_char);
int		ipshowhdr(void *, ipcheat *);
Bool		isbrc(IPaddr);
