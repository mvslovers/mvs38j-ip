...
//  I changed the u_short to unsigned int in several places
//  to work with Systems/C option -fansi_bitfield.
//  I don't believe MVS-Xinu makes much, if any, use of
...
struct le_device {
	...
	unsigned int	:14;		/* Reserved jmm */
	unsigned int	le_rap	: 2;	/* Register Address Port jmm */
};
...
struct le_drp {			/* Descriptor Ring Pointer */
	...
	u_int	drp_len	: 3;	/* Binary exponent of no. of ring entries jmm */
	u_int		: 5;	/* Reserved jmm */
...
struct le_init_block {
	...
	u_short			: 7;	/* Reserved */
	u_int	ib_intl	: 1;	/* Internal Loopback jmm*/
	u_int	ib_drty	: 1;	/* Disable Retry jmm */
	u_int	ib_coll	: 1;	/* Force Collision jmm */
	u_int	ib_dtcr	: 1;	/* Disable Transmit CRC jmm */
	u_int	ib_loop	: 1;	/* Loopback jmm */
	u_int	ib_dtx	: 1;	/* Disable Transmitter jmm */
	u_int	ib_drx	: 1;	/* Disable Receiver jmm */
...
struct	le_md {			/* Message Descriptor */
	...
	u_char	lmd_flags;
	u_int	lmd_hadr: 8;	/* High Order 8 Address Bits jmm */
