...
//  TODO remove IP2NAME_FAKE from Configuration when DNS available
...
char	*ip2name(IPaddr ip, char *nam) {
	...
	ip2dot(bufip, ip);
	trace("\nIP2NAME enter; IP %s nam 0x%x\n", bufip, nam);

#ifdef	IP2NAME_FAKE
	strcpy(nam, "bogus.com");
	trace("IP2NAME fake exit; nam 0x%x\n", nam);
	return nam;
#endif
	...
	*p++ = NULLCH;	/* terminate name */
	...
	if (i == TIMEOUT) {
		close(dg);
		temp = ip2dot(nam, ip);
		trace("IP2NAME timeout; return ip2dot %s\n", temp);
		return temp;
	}
	close(dg);
